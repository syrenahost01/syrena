...
sleep 15
sudo docker exec -it ${CONTAINER_NAMESPACE}-kafka-zookeeper kafka-topics --create --topic ${LOCAL_TOPIC} --partitions 1 --replication-factor 1 --config cleanup.policy=${KAFAKA_CLEANUP_POLICY} --config compression.type=${KAFKA_COMPRESSION_TYPE} --retention.ms=${KAFKA_RETENTION_MS} --if-not-exists --zookeeper ${CONTAINER_NAMESPACE}-kafka-zookeeper:2181
