...
sleep 15
sudo docker exec -it ${CONTAINER_NAMESPACE}-kafka-zookeeper kafka-topics --create --topic ${LOCAL_TOPIC} --partitions 1 --replication-factor 1 --if-not-exists --zookeeper ${CONTAINER_NAMESPACE}-kafka-zookeeper:2181
sudo docker exec -it ${CONTAINER_NAMESPACE}-kafka-zookeeper kafka-topics --create --topic ${REMOTE_TOPIC} --partitions 1 --replication-factor 1 --if-not-exists --zookeeper ${CONTAINER_NAMESPACE}-kafka-zookeeper:2181
